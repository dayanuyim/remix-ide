...
function runTab (appAPI = {}, appEvents = {}, opts = {}) {
  ...
  self._view = {}
  self.data = { count: 0 }
  self._view.txRecorderCount = yo`<span>${self.data.count} Events Recorded</span>`
  ...
  self._view.noInstancesText = yo`<div class="${css.noInstancesText}">Currently you have no contract instances to interact with.</div>`
  ...
  self.recorderOpts = {
    ...
    collapsedView: () => {
      ...
      <div class=${css.recorderContainer}>
        <div class=${css.recorderContainerTitle} title="aa">
  ...
  <div>
    ...
    ${contractDropdown(event, appAPI, appEvents, self)}
    ${recorderCard.render()}
...
function makeRecorder (events, appAPI, appEvents, self) {
  ...
  function triggerRecordButton () {
    ...
    modalDialogCustom.prompt(null, 'Save created transactions and reply them in a different environment (e.g. form JS VM to test net). The file will be saved under ' + path, 'scenario.json', input => {
        ...
        if (currentFile.match('.json$')) {
          ...
          if (txArray.length) {
            ...
            recorder.run(txArray, accounts, options, abis, linkReferences, (abi, address, contractName) => {
              self._view.instanceContainer.appendChild(appAPI.udappUI().renderInstanceFromABI(abi, address, contractName))
            })
          }
        } else {
...
function contractDropdown (events, appAPI, appEvents, self) {
  var instanceContainer = self._view.instanceContainer
  var instanceContainerTitle = self._view.instanceContainerTitle
  instanceContainer.appendChild(instanceContainerTitle)
  instanceContainer.appendChild(self._view.noInstancesText)
      ...
      if (!error) {
        ...
        appAPI.udapp().createContract(data, (error, txResult) => {
          if (!error) {
            ...
            self._view.instanceContainer.appendChild(appAPI.udappUI().renderInstance(selectedContract.contract.object, address, selectContractNames.value))
          } else {
  ...
  function loadFromAddress (appAPI) {
    ...
    if (/.(.abi)$/.exec(appAPI.currentFile())) {
      modalDialogCustom.confirm(null, 'Do you really want to interact with ' + address + ' using the current ABI definition ?', () => {
        ...
        self._view.instanceContainer.appendChild(appAPI.udappUI().renderInstanceFromABI(abi, address, address))
      })
    } else {
      var contract = appAPI.getContract(contractNames.children[contractNames.selectedIndex].innerHTML)
      self._view.instanceContainer.appendChild(appAPI.udappUI().renderInstance(contract.object, address, selectContractNames.value))
    }
  }
